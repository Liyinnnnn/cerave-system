        ...
        <div class="bg-white dark:bg-slate-800 border-b border-gray-200 dark:border-slate-700 px-6 py-4 shadow-sm">
            <div class="flex items-center justify-between">
                ...
                <div class="flex gap-2">
                    @if($session)
                    <a href="{{ route('dr-c.viewReport', ['session' => $session->id]) }}" target="_blank" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition font-medium flex items-center gap-2 shadow-md">
                        <i class="ri-file-text-line"></i>
                        <span>View Report</span>
                    </a>
                    @endif
                    <a href="{{ route('dr-c.chat', ['new' => 1]) }}" class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition font-medium flex items-center gap-2 shadow-md">
                                ...
                                @else
                                    <div class="flex justify-start">
                                        <div class="max-w-3xl bg-white dark:bg-slate-700 border border-gray-200 dark:border-slate-600 rounded-2xl px-5 py-3 shadow-md">
                                            <p class="text-sm leading-relaxed text-gray-800 dark:text-white">{{ $msg->message }}</p>
                                            <span class="text-xs text-gray-500 dark:text-gray-400 mt-1 block">Dr. C • {{ $msg->created_at->format('H:i') }}</span>
                                        </div>
                                    </div>
                                @endif
                            @endforeach
