            ...
            <div class="flex items-center justify-between mb-4">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        @if($session)
                            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Session: {{ $session->concerns ?? 'General' }}</p>
                        @endif
                    </div>
                </div>
                @auth
                <div class="flex gap-3">
                    @if($session)
                    <a href="{{ route('dr-c.viewReport', ['session' => $session->id]) }}" target="_blank" class="px-4 py-2 bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-lg hover:from-blue-600 hover:to-blue-700 transition font-medium flex items-center gap-2 shadow-md">
                        <i class="ri-file-text-line"></i>
                        <span>View Report</span>
                    </a>
                    @endif
                    <a href="{{ route('dr-c.chat', ['new' => 1]) }}" class="px-4 py-2 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-lg hover:from-green-600 hover:to-emerald-700 transition font-medium flex items-center gap-2 shadow-md">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <div class="lg:col-span-1">
                <div class="bg-white dark:bg-gradient-to-br dark:from-slate-900 dark:to-indigo-950 dark:border dark:border-slate-800 rounded-xl shadow-lg p-6">
                    ...
                    @if(isset($recentSessions) && $recentSessions->count())
                        <div class="space-y-3">
                            @foreach($recentSessions as $s)
                                <a href="{{ route('dr-c.chat', ['session' => $s->session_token]) }}" 
                                   class="block p-3 rounded-lg transition border {{ $session && $session->id === $s->id ? 'bg-blue-100 dark:bg-blue-900 border-blue-300 dark:border-blue-700' : 'bg-gray-50 dark:bg-slate-800 hover:bg-blue-50 dark:hover:bg-slate-700 border-gray-200 dark:border-slate-700' }}">
                                    <div class="flex justify-between text-xs text-gray-500 dark:text-gray-400 mb-1">
                                        <span>#{{ $s->id }}</span>
                                        <span>{{ $s->created_at->format('M d, H:i') }}</span>
                                    </div>
                                    <p class="text-sm font-medium text-gray-800 dark:text-white truncate">{{ $s->concerns ?? 'General' }}</p>
                                    <p class="text-xs text-gray-500 dark:text-gray-400">{{ $s->messages_count }} msgs • {{ $s->status }}</p>
                                </a>
                            @endforeach
                        </div>
                    @else
            ...
            <!-- Main Chat -->
            <div class="@auth lg:col-span-3 @else lg:col-span-4 @endauth">
                                ...
                                @if($msg->message_type === 'user')
                                    <div class="flex justify-end">
                                        <div class="max-w-md bg-blue-600 text-white rounded-lg p-4 shadow">
                                            ...
                                            <span class="text-xs opacity-75 mt-1 block">{{ $msg->created_at->format('H:i') }}</span>
                                        </div>
                                    </div>
                                @else
                                    <div class="flex justify-start">
                                        <div class="max-w-md bg-white dark:bg-slate-700 border border-gray-200 dark:border-slate-600 rounded-lg p-4 shadow">
                                            <p class="text-sm text-gray-800 dark:text-white">{{ $msg->message }}</p>
                    ...
                    <div class="px-6 py-4 bg-white dark:bg-slate-800 border-t border-gray-200 dark:border-slate-700">
                        <form id="chatForm" class="flex gap-3">
                            ...
                            <input type="text" 
                                   name="message" 
                                   id="messageInput" 
                                   placeholder="{{ $session ? 'Continue your conversation...' : 'Tell Dr. C about your skin concerns...' }}"
                        ...
                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-2">
                            @if(isset($rateLimitRemaining))
                                {{ $rateLimitRemaining }} messages remaining this hour
                            @else
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    chatForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            console.log('Sending to:', '{{ route("dr-c.send") }}');
            ...
            const response = await fetch('{{ route("dr-c.send") }}', {
                method: 'POST',
                headers: {
