...
<head>
    ...
    <title>CeraVe Skincare Consultation Report - {{ $session->user->name ?? 'Guest' }}</title>
    <style>
        @media print {
...
<body>
    <a href="{{ route('dr-c.chat') }}" class="back-button no-print">← Back to Dr. C</a>
    ...
    <div class="container">
        ...
        <div class="report-meta">
            <strong>Report ID:</strong> {{ $session->session_token }} &nbsp;|&nbsp; 
        ...
        <div class="content">
            ...
            <div class="section">
                ...
                <div class="info-list">
                    <div class="info-list-item">
                        ...
                        <span class="info-value">{{ $session->user->name ?? 'Guest' }}</span>
                    </div>
                    <div class="info-list-item">
                        <span class="info-label">Consultation Date</span>
                        <span class="info-value">{{ $session->created_at->format('F d, Y') }}</span>
                    </div>
                    <div class="info-list-item">
                        <span class="info-label">Consultation Time</span>
                        <span class="info-value">{{ $session->created_at->format('h:i A') }}</span>
                    </div>
                    <div class="info-list-item">
                    ...
                    <div class="info-list-item">
                        ...
                        <span class="info-value" style="text-transform: capitalize;">{{ $session->status }}</span>
                    </div>
                    <div class="info-list-item">
                        <span class="info-label">Total Messages Exchanged</span>
                        <span class="info-value">{{ $session->message_count }} messages</span>
                    </div>
                    <div class="info-list-item">
            ...
            <div class="section">
                ...
                <div style="margin-bottom: 25px;">
                    ...
                    <div class="assessment-box">{{ $session->concerns }}</div>
                </div>
                @endif
            </div>
            ...
            <div class="section">
                ...
                <div class="recommendations-box">{!! nl2br(e($session->report)) !!}</div>
            </div>
            @endif
                    ...
                    @foreach($recommendedProducts as $product)
                        <div class="product-card">
                            @if($product->image_url)
                                <img src="{{ asset($product->image_url) }}" alt="{{ $product->name }}" class="product-image" onerror="this.onerror=null; this.style.display='none'; this.nextElementSibling.style.display='flex';">
                            ...
                            @elseif($product->images && is_array($product->images) && count($product->images) > 0)
                                <img src="{{ asset($product->images[0]) }}" alt="{{ $product->name }}" class="product-image" onerror="this.onerror=null; this.style.display='none'; this.nextElementSibling.style.display='flex';">
                                <div class="product-image-placeholder" style="display: none;">📦</div>
                            @else
                            ...
                            <div class="product-info">
                                ...
                                @if($product->key_benefits)
                                    <div class="product-description">{{ Str::limit($product->key_benefits, 120) }}</div>
                                @elseif($product->benefits)
                                    <div class="product-description">{{ Str::limit($product->benefits, 120) }}</div>
                                @elseif($product->description)
                                    <div class="product-description">{{ Str::limit($product->description, 120) }}</div>
                                @endif
                            </div>
                        </div>
                    @endforeach
        ...
        <div class="footer">
            ...
            <p style="margin-bottom: 10px; font-size: 13px; line-height: 1.6;">
                This consultation report has been prepared for <strong>{{ $session->user->name ?? 'you' }}</strong>.
            </p>
            ...
            <p style="color: #94a3b8; margin-top: 15px;">
                Report Generated: {{ now()->format('F d, Y \a\t h:i A') }} &nbsp;|&nbsp; 
                Document ID: {{ $session->session_token }} &nbsp;|&nbsp;
                © {{ now()->year }} CeraVe Skincare
            </p>
        </div>
    </div>
</body>
...
<body>
    ...
    <div class="container">
            ...
            <div class="section">
                ...
                <div class="info-grid">
                    <div class="info-item">
                        ...
                        <div class="info-value">{{ $session->user->name ?? 'Guest User' }}</div>
                    </div>
                    <div class="info-item">
                    ...
                    <div class="info-item">
                        ...
                        <div class="info-value">{{ $session->created_at->format('F d, Y') }}</div>
                    </div>
                    <div class="info-item">
                        <div class="info-label">Consultation Time</div>
                        <div class="info-value">{{ $session->created_at->format('h:i A') }}</div>
                    </div>
                    <div class="info-item">
                    ...
                    <div class="info-item">
                        ...
                        <div class="info-value" style="text-transform: capitalize;">{{ $session->status }}</div>
                    </div>
                </div>
            </div>
            ...
            <div class="section">
                ...
                <div class="info-grid">
                    <div class="info-item">
                        ...
                        <div class="info-value">{{ $session->concerns ?? 'General skincare consultation' }}</div>
                    </div>
                    <div class="info-item">
                        <div class="info-label">Messages Exchanged</div>
                        <div class="info-value">{{ $session->message_count }} messages</div>
                    </div>
                </div>
            </div>
            ...
            <div class="section">
                ...
                <div class="conversation-box">
                    @foreach($messages as $message)
                        <div class="message {{ $message->message_type === 'user' ? 'user-message' : 'assistant-message' }}">
                            <div class="message-label">
                                {{ $message->message_type === 'user' ? $session->user->name ?? 'Patient' : 'Dr. C' }}
                                <span style="font-weight: 400; margin-left: 8px;">{{ $message->created_at->format('h:i A') }}</span>
                            </div>
        ...
        <div class="footer">
            ...
            <p style="color: #9ca3af; margin-top: 5px;">Report ID: {{ $session->session_token }}</p>
        </div>
    </div>
</body>
</html>
