...
<div class="py-10 bg-gray-50">
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white shadow-lg rounded-2xl p-8">
            ...
            <form id="productForm" action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8" onsubmit="return prepareFormSubmission()">
                @csrf
                ...
                <div class="border-b border-gray-200 pb-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="name" value="{{ old('name') }}" class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition" required placeholder="e.g. Moisturising Cream">
                            @error('name')<p class="text-sm text-red-500 mt-1">{{ $message }}</p>@enderror
                        </div>
                        
                        <div>
                            <label class="block text-sm font-bold text-gray-800 mb-2 uppercase tracking-wide">🏷️ Category *</label>
                            <input type="text" name="category" value="{{ old('category') }}" list="categoryList" class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition" required placeholder="Select existing or type new category">
                                ...
                                @foreach (['dry','oily','combination','sensitive','normal'] as $type)
                                    <label class="flex items-center space-x-3 cursor-pointer hover:bg-gray-100 p-2 rounded transition">
                                        <input type="checkbox" name="skin_types[]" value="{{ $type }}" 
                                            @checked(is_array(old('skin_types')) && in_array($type, old('skin_types')))
                        ...
                        <div class="image-input-group flex gap-2">
                            <select name="images[]" class="flex-1 px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition image-select bg-white">
                                ...
                                @foreach ($imageFiles as $file)
                                    <option value="/images/{{ $file }}" @selected(old('images.0') === "/images/{$file}")/images/{{ $file }}</option>
                                @endforeach
                            </select>
                            ...
                            <div class="space-y-3">
                                <div class="flex gap-2">
                                    <div class="flex-1">
                                        ...
                                        <input type="text" name="retailer_names[]" value="{{ old('retailer_names.0') }}" class="w-full px-4 py-2 mt-1 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition" placeholder="e.g., Shopee">
                                    </div>
                                    <div class="flex-1">
                                        <label class="text-xs font-semibold text-gray-600 uppercase">Purchase URL</label>
                                        <input type="url" name="external_urls[]" value="{{ old('external_urls.0') }}" class="w-full px-4 py-2 mt-1 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition" placeholder="https://store.com/product">
                                    </div>
                ...
                <div class="border-b border-gray-200 pb-8">
                    ...
                    <textarea name="benefits" id="benefitsInput" class="hidden">{{ old('benefits') }}</textarea>
                ...
                <div class="border-b border-gray-200 pb-8">
                        ...
                        <div class="feature-card-group bg-gradient-to-br from-gray-50 to-gray-100 p-5 rounded-lg border-2 border-purple-200">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                ...
                                <div>
                                    ...
                                    <textarea name="features[0][text]" id="featureTextInput0" class="hidden">{{ old('features.0.text') }}</textarea>
                                </div>
                            </div>
                    ...
                    @error('features')<p class="text-sm text-red-500 mt-2">{{ $message }}</p>@enderror
                </div>
                ...
                <div class="border-b border-gray-200 pb-8">
                    ...
                    <textarea name="ingredients" id="ingredientsInput" class="hidden">{{ old('ingredients') }}</textarea>
                ...
                <div class="pb-8">
                    ...
                    <textarea name="directions" id="directionsInput" class="hidden">{{ old('directions') }}</textarea>
                    @error('directions')<p class="text-sm text-red-500 mt-1">{{ $message }}</p>@enderror
                </div>

                <!-- Form Actions -->
                <div class="flex justify-end gap-4 pt-6 border-t border-gray-200">
                    <a href="{{ route('products.index') }}" class="px-6 py-3 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition font-semibold">Cancel</a>
