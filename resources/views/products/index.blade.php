...
<div class="py-16 bg-gray-50 min-h-screen dark:bg-indigo-900">
    <div class="container mx-auto px-4">
        ...
        <div class="text-center mb-12">
            ...
            @auth
                @if (auth()->user()->isAdmin())
                    <a href="{{ route('products.create') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-blue-600 text-white rounded-lg shadow hover:bg-blue-700 font-semibold">
        ...
        <div class="mb-8 bg-white rounded-xl shadow-lg p-6 dark:bg-gradient-to-br dark:from-slate-900 dark:to-indigo-950 dark:border dark:border-slate-800">
            <form action="{{ route('products.index', [], false) }}" method="GET" class="space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <!-- Search -->
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Search</label>
                        <input type="text" name="search" placeholder="Search products..." value="{{ $search ?? '' }}" onchange="this.form.submit()"
                ...
                <div class="flex justify-center">
                    <a href="{{ route('products.index', [], false) }}" class="px-6 py-2 bg-gray-200 text-gray-700 dark:bg-slate-700 dark:text-gray-200 rounded-lg hover:bg-gray-300 dark:hover:bg-slate-600 transition-all duration-200 font-semibold">
                        Clear All
                    </a>
                </div>

                <!-- Active Filters Display -->
                @if($search || $category || $skinType)
        ...
        <div class="mb-4 text-gray-600 dark:text-blue-200">
            <p>Showing <span class="font-semibold">{{ $products->firstItem() ?? 0 }}</span> to <span class="font-semibold">{{ $products->lastItem() ?? 0 }}</span> of <span class="font-semibold">{{ $products->total() }}</span> products</p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-8 mb-12">
            @forelse ($products as $product)
                <div class="bg-white rounded-lg shadow-md overflow-hidden transition-transform hover:shadow-lg hover:-translate-y-1 dark:bg-gradient-to-br dark:from-indigo-900 dark:to-indigo-800 dark:border dark:border-indigo-800">
                    <a href="{{ route('products.show', $product, false) }}" class="block cursor-pointer">
                        ...
                        <div class="p-6">
                            <div class="flex justify-between items-center mb-2">
                                <span class="bg-blue-100 text-blue-600 text-xs px-3 py-1 rounded-full font-semibold dark:bg-blue-900 dark:text-blue-200">{{ $product->category ?? 'Featured' }}</span>
                                <div class="flex">
                                    @php $rating = round($product->rating ?? 4.5); @endphp
                                    @for ($i = 1; $i <= 5; $i++)
                                        @if ($i <= $rating)
                            ...
                            <p class="text-gray-600 text-sm mb-4 dark:text-blue-200">{{ Str::limit(strip_tags($product->description), 60) }}</p>
                        </div>
                    </a>
                    ...
                    @auth
                        @if (auth()->user()->isAdmin())
                            <div class="px-6 pb-6 flex gap-2 border-t pt-4">
                                <a href="{{ route('products.edit', $product, false) }}" class="flex-1 px-3 py-2 bg-blue-100 text-blue-600 rounded hover:bg-blue-200 text-center text-sm font-medium transition" onclick="event.stopPropagation();">
                                    <i class="ri-edit-line"></i> Edit
                                </a>
                                <form action="{{ route('products.destroy', $product, false) }}" method="POST" class="flex-1" onsubmit="event.stopPropagation(); return confirm('Delete this product?');">
                                    @csrf
                                    @method('DELETE')
