...
@section('content')
    ...
    @if (session('form') === 'password' && session('success'))
        <div class="fixed top-4 right-4 z-[9999] bg-green-50 border-l-4 border-green-500 p-4 rounded-lg shadow-2xl max-w-md">
            <div class="flex items-center gap-3">
                <svg class="w-6 h-6 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
    ...
    @if (session('form') === 'password' && $errors->getBag('password')->any())
    ...
    @if (session('form') === 'skincare' && session('success'))
    ...
    @if (session('form') === 'profile' && session('success'))
        <div class="fixed top-4 right-4 z-50 bg-green-50 border-l-4 border-green-500 p-4 rounded-lg shadow-lg animate-slide-in-right max-w-md">
            <div class="flex items-center gap-3">
                <svg class="w-6 h-6 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
    ...
    @if (session('form') === 'profile' && $errors->any())
        ...
        <div class="max-w-6xl mx-auto flex bg-white shadow-2xl rounded-2xl overflow-hidden dark:bg-slate-900">
            ...
            <aside class="w-1/4 bg-gradient-to-b from-blue-900 to-blue-800 text-white p-6 relative dark:from-slate-900 dark:to-indigo-950 dark:border-r dark:border-slate-800">
                ...
                <div class="flex flex-col items-center text-center space-y-3 relative z-10">
                    ...
                    <h3 class="text-xl font-bold dark:text-white">Hey, {{ $user->nickname }}!</h3>
                    <p class="text-sm text-blue-100 font-medium dark:text-blue-200">{{ $user->name }}</p>
                    <p class="text-xs text-blue-200 flex items-center gap-2 dark:text-blue-300">
                        ...
                        <span>{{ \Carbon\Carbon::parse($user->birthday)->format('jS M Y') }}</span>
                    </p>
                    <p class="text-xs text-blue-200 break-all px-2 dark:text-blue-300">{{ $user->email }}</p>
                </div>
            ...
            <div class="w-3/4 p-10 space-y-10 bg-gray-50 dark:bg-indigo-900">
                @if(is_null($user->password))
                    <section class="scroll-mt-4">
                        <div class="bg-yellow-50 rounded-xl shadow-lg p-6 border-2 border-yellow-200 dark:bg-indigo-900 dark:bg-opacity-60 dark:border-indigo-700">
                            ...
                            <form method="POST" action="{{ route('profile.password.set') }}" class="space-y-4">
                                @csrf
                                <div>
                            ...
                            <div class="grid grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <select id="gender" name="gender" required
                                        ...
                                        <option value="Male"
                                            {{ old('gender', strtolower($user->gender)) === 'male' ? 'selected' : '' }}>Male
                                        </option>
                                        <option value="Female"
                                            {{ old('gender', strtolower($user->gender)) === 'female' ? 'selected' : '' }}>
                                            Female</option>
                                    </select>
                                ...
                                <div>
                                    ...
                                    <div class="flex gap-2">
                                        <select name="country_code" class="w-32 border-2 border-gray-200 p-3 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all dark:bg-white dark:text-gray-800 dark:border-gray-200 dark:focus:border-blue-400 dark:focus:ring-blue-300">
                                            <option value="+60" {{ old('country_code', $user->country_code ?? '+60') === '+60' ? 'selected' : '' }}>🇲🇾 +60</option>
                                            ...
                                            <option value="+61" {{ old('country_code', $user->country_code) === '+61' ? 'selected' : '' }}>🇦🇺 +61</option>
                                            <option value="+65" {{ old('country_code', $user->country_code) === '+65' ? 'selected' : '' }}>🇸🇬 +65</option>
                                            <option value="+86" {{ old('country_code', $user->country_code) === '+86' ? 'selected' : '' }}>🇨🇳 +86</option>
                                            <option value="+91" {{ old('country_code', $user->country_code) === '+91' ? 'selected' : '' }}>🇮🇳 +91</option>
                                            <option value="+81" {{ old('country_code', $user->country_code) === '+81' ? 'selected' : '' }}>🇯🇵 +81</option>
                                            <option value="+82" {{ old('country_code', $user->country_code) === '+82' ? 'selected' : '' }}>🇰🇷 +82</option>
                                            <option value="+66" {{ old('country_code', $user->country_code) === '+66' ? 'selected' : '' }}>🇹🇭 +66</option>
                                            <option value="+84" {{ old('country_code', $user->country_code) === '+84' ? 'selected' : '' }}>🇻🇳 +84</option>
                                            <option value="+63" {{ old('country_code', $user->country_code) === '+63' ? 'selected' : '' }}>🇵🇭 +63</option>
                                            <option value="+62" {{ old('country_code', $user->country_code) === '+62' ? 'selected' : '' }}>🇮🇩 +62</option>
                                        </select>
                ...
                <section id="skincare" class="scroll-mt-4">
                    <div class="bg-white rounded-xl shadow-lg p-8 border border-gray-100 dark:bg-gradient-to-br dark:from-slate-900 dark:to-indigo-950 dark:border-slate-800">
                        ...
                        <form action="{{ route('profile.skincare.update') }}" method="POST" class="space-y-6">
                            @csrf
                            @method('patch')
                            ...
                            <div class="grid grid-cols-1 gap-6">
                                <div>
                                    ...
                                    <select name="skin_type" class="w-full border-2 border-gray-200 p-3 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all dark:bg-white dark:text-gray-800 dark:border-gray-200 dark:focus:border-blue-400 dark:focus:ring-blue-300">
                                        ...
                                        <option value="Normal" {{ old('skin_type', $user->skin_type) === 'Normal' ? 'selected' : '' }}>Normal</option>
                                        <option value="Dry" {{ old('skin_type', $user->skin_type) === 'Dry' ? 'selected' : '' }}>Dry</option>
                                        <option value="Oily" {{ old('skin_type', $user->skin_type) === 'Oily' ? 'selected' : '' }}>Oily</option>
                                        <option value="Combination" {{ old('skin_type', $user->skin_type) === 'Combination' ? 'selected' : '' }}>Combination</option>
                                        <option value="Sensitive" {{ old('skin_type', $user->skin_type) === 'Sensitive' ? 'selected' : '' }}>Sensitive</option>
                                    </select>
                                    @error('skin_type')
                                        <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                                    @enderror
                                        ...
                                        @foreach($products as $product)
                                            <label class="flex items-start gap-3 p-3 border border-gray-200 rounded-lg hover:bg-blue-50 cursor-pointer transition-all dark:border-gray-300 dark:hover:bg-blue-100">
                                                ...
                                                <div class="flex-1 min-w-0">
                                                    ...
                                                    <p class="text-xs text-gray-500 dark:text-gray-600">{{ $product->category }}</p>
                                                </div>
                                            </label>
                                        @endforeach
                                    </div>
                                    @error('using_products')
                                        <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                                    @enderror
                                </div>
                            </div>
